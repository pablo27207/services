...
TASKS = {
    "buoy": {
        ...
        "db_insert": lambda db, data: db.insert_buoy_data(data),
        "schedule": crontab(minute=0)  # Cada hora
    },
    "mareograph": {
        "scraper": MareographScraper.fetch_mareograph_data,
        "db_insert": lambda db, data: db.insert_mareograph_data(data),
        "schedule": crontab(minute="*/10")  # Cada 10 min
    },
    "tide_forecast": {
        "scraper": TideScraper.fetch_tide_data,
        "db_insert": lambda db, data: db.insert_tide_forecast(data),
...
# 2. Definir en db_handler.py un metodo insert_<NOMBRE>_data
# 3. Agregar en este archivo el nuevo scraper con la siguiente estructura:
# "<NOMBRE>_scraper": {
#     "scraper": <NOMBRE>Scraper.fetch_<NOMBRE>_data,
#     "db_insert": lambda db, data: db.insert_<NOMBRE>_data(data),
